...
describe('Todo routes', function () {
	...
	describe('`/users` URI', function () {
		...
		it('GET responde con todo el que tenga una tarea', function () {
			todos.add('manu', {content: 'comprar medialunas'});
			todos.add('franco', {content: 'hacer un chivito'});
			todos.add('emi', {content: 'comprar caramelos'});
			return supertest
				.get('/users')
				.expect(200)
				.expect('Content-Type', /json/)
				.expect(function (res) {
					expect(res.body).to.eql(['manu', 'franco', 'emi']);
				});
		});
	});
	...
	describe('`/users/:name/tasks` URI', function () {
		...
		describe('filtro de query (?key=value)', function () {
			beforeEach(function () {
				...
				todos.add('solano', {
					content: 'habilitar request para tareas especificas',
				});
			});
				...
				return supertest
					...
					.expect(function (res) {
						...
						expect(res.body[0].content).to.equal(
							'aprender que tiene req.query'
						); // y es la tarea completada
					});
			});
			...
			it('GET podemos pedir solo las tareas activas (incompletas)', function () {
				return supertest
					...
					.expect(function (res) {
						...
						expect(res.body[0].content).to.equal(
							'habilitar request para tareas especificas'
						); // y es la tarea activa
					});
			});
		});
		...
		describe('`/:index` URI', function () {
			it('PUT asigna una tarea como completa', function () {
				todos.add('manu', {content: 't0'});
				todos.add('manu', {content: 't1'});
				todos.add('manu', {content: 't2'});

				return supertest
					.put('/users/manu/tasks/1') // indica con el indice a que tarea apunto
					.expect(200)
					.expect(function () {
						expect(todos.list('manu')[0].complete).to.be.false;
						expect(todos.list('manu')[1].complete).to.be.true; // y se completa
						expect(todos.list('manu')[2].complete).to.be.false;
					});
			});

			it('DELETE borra una tarea', function () {
				todos.add('franco', {content: 'dar lecture'});
				todos.add('franco', {content: 'ayudar con el workshop'});
				todos.add('franco', {content: 'hacer el review'});

				return supertest
					.delete('/users/franco/tasks/1')
					.expect(204)
					.expect(function () {
						expect(todos.list('franco')).to.have.length(2);
						expect(todos.list('franco')[0].content).to.equal('dar lecture');
						expect(todos.list('franco')[1].content).to.equal('hacer el review');
					});
			});
		});

		describe('manejo de errores', function () {
			it('responde con status 404 si el usuario no existe', function () {
				return supertest.get('/users/messi/tasks').expect(404);
			});
